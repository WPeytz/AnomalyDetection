...
def create_anomaly_heatmap(patch_scores: np.ndarray, patch_size: int = 16, image_size: int = 224) -> np.ndarray:
    ...
    Returns:
        2D anomaly heatmap
    """
    # DINOv3 with image_size=224 and patch_size=16 creates a 14x14 grid (196 patches)
    # But it may have additional tokens, so we need to find the correct grid size
    ...
    if grid_size * grid_size == n_patches:
        ...
        # For 224x224 image with 16x16 patches, we should have 14x14 = 196 patches
        expected_grid_size = image_size // patch_size
        expected_patches = expected_grid_size * expected_grid_size
        
        if n_patches > expected_patches:
            # We have extra patches (possibly due to different implementation)
            # Use the first expected_patches
            heatmap = patch_scores[:expected_patches].reshape(expected_grid_size, expected_grid_size)
        elif n_patches < expected_patches:
            # We have fewer patches - pad with zeros
            padded_scores = np.zeros(expected_patches)
            padded_scores[:n_patches] = patch_scores
...
def visualize_sam_enhanced_results(
    ...
    n_plots = 2 if (anomaly_heatmap is None and sam_mask is None) else (3 if sam_mask is None else 4)
...
def run_sam_enhanced_anomaly_detection(
    ...
    output_dir = Path(output_dir) / f"{category}{sam_suffix}{shots_suffix}"
    ...
    if use_patches:
        # Image-level anomaly scores for detection
        ...
        # Compute patch-level scores for heatmap visualization
...
def main():
    ...
    parser.add_argument(
        "--root-dir",
        type=str,
        default="/zhome/9c/f/221532/Deep_Learning_Project_SAM/mvtec_ad",
    ...
    parser.add_argument(
        "--use-sam-masking",
        action="store_true",
        help="Use SAM for masking normal objects in prototypes"
    )
    parser.add_argument(
        "--few-shot",
        action="store_true",
